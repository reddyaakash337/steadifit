...
export function HomeScreen() {
  ...
  return <Screen>
    ...
    <SectionTitle title="Progress" action="View" onPress={() => router.push('/(tabs)/progress')} /><View style={s.statsRow}><Stat value={`${state.streak}`} label="Day streak" /><Stat value={`${state.history.length}`} label="Workouts" /><Stat value={`${state.history[0]?.volume.toLocaleString() ?? '0'} ${state.units}`} label="Last volume" /></View>
    <SectionTitle title="Recent workout" action="History" onPress={() => router.push('/history')} />
    {recent ? <Card onPress={() => openHistoryItem(recent.id)}><Text style={s.cardTitle}>{recent.name}</Text><Copy>{recent.date} · {recent.duration} min · {recent.volume.toLocaleString()} {state.units}</Copy></Card> : <Empty title="No workouts yet" detail="Complete your first workout to start building your history." />}
...
export function WorkoutHistoryDetailsScreen({ id }: { id: string }) {
  ...
  return <Screen><TopBar title="Workout details" /><Heading>{item.name}</Heading><Copy>{item.date} · {item.duration} min · {item.volume.toLocaleString()} {state.units} total volume</Copy>{item.personalRecord ? <View style={{ marginTop: 9 }}><Pill green>New PR</Pill></View> : null}{item.exercises.map(ex => { const unit = exerciseById(ex.exerciseId).repRange.toLowerCase().includes('min') ? 'min' : exerciseById(ex.exerciseId).repRange.toLowerCase().includes('sec') ? 'sec' : 'reps'; return <Card key={ex.exerciseId}><Text style={s.cardTitle}>{exerciseById(ex.exerciseId).name}{ex.skipped ? ' · Skipped' : ''}</Text>{ex.sets.length ? ex.sets.map((set, index) => <Copy key={index}>Set {index + 1} · {set.weight || 'Bodyweight'}{set.weight ? ` ${state.units}` : ''} × {set.reps} {unit}</Copy>) : <Copy>{ex.skipped ? 'Skipped during this session' : 'No completed sets'}</Copy>}</Card>; })}<Action title="Repeat workout" onPress={() => item.workoutId.startsWith('exercise-') ? router.push({ pathname: '/exercises/[id]', params: { id: item.workoutId.slice('exercise-'.length) } }) : router.push({ pathname: '/workout/[id]', params: { id: item.workoutId } })} /></Screen>;
}
...
export function ProgressScreen() {
  ...
  const thisWeek = state.history.filter(item => (item.completedAt ?? 0) >= weekStart.getTime());
  const bestBench = state.history.flatMap(item => item.exercises).filter(ex => ex.exerciseId === 'bench').flatMap(ex => ex.sets).reduce((best, set) => Math.max(best, set.weight), 0);
  const records = state.history.filter(item => item.personalRecord);
  return <Screen><Heading>Progress</Heading><Copy>Your training, at a glance.</Copy><View style={s.statsRow}><Stat value={`${thisWeek.length}`} label="This week" /><Stat value={`${state.streak}`} label="Day streak" /><Stat value={`${thisWeek.reduce((sum, item) => sum + item.volume, 0).toLocaleString()} ${state.units}`} label="Weekly volume" /></View><SectionTitle title="Strength" /><Card><Text style={s.cardTitle}>Bench Press</Text><Text style={s.chartValue}>{bestBench ? `${bestBench} ${state.units}` : 'No logged sets'}</Text><Copy>{bestBench ? 'Best weight in your completed sessions' : 'Complete a workout with bench press to start tracking.'}</Copy><View style={s.chartBars}>{[35, 44, 50, 56, 68, 72, 82, 92].map((height, index) => <View key={index} style={[s.bar, { height }]} />)}</View></Card><SectionTitle title="Personal records" />{records.length ? records.map(item => <Card key={item.id}><Text style={s.cardTitle}>{item.name}</Text><Copy>{item.date} · logged personal record</Copy></Card>) : <Empty title="No new records yet" detail="Your logged personal records will appear here." />}<SectionTitle title="Body" /><Card><Text style={s.cardTitle}>Weight trend</Text><Copy>Current mock weight: 80.2 kg</Copy></Card><Action title="Workout history" secondary onPress={() => router.push('/history')} /></Screen>;
}
