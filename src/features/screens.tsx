...
export function HomeScreen() {
  ...
  return <Screen>
    ...
    <Card style={s.hero} onPress={() => planDay && router.push({ pathname: '/plan/day/[day]', params: { day: String(planDay.day) } })}><Eyebrow>{isTrainingDay ? 'TODAY’S WORKOUT' : 'NEXT UP IN YOUR PLAN'}</Eyebrow><Text style={s.heroTitle}>{workoutName}</Text><Text style={s.heroMeta}>{planDay ? planDayFocus(planDay) : workout.focus} · {duration} min · {planDay ? plannedExercises(planDay).length : workout.exerciseIds.length} exercises</Text><Action title="View workout  ›" onPress={() => planDay ? router.push({ pathname: '/plan/day/[day]', params: { day: String(planDay.day) } }) : pushWorkout(workout.id)} /><Pressable onPress={() => { if (state.activeWorkout) router.push({ pathname: '/active/[id]', params: { id: state.activeWorkout.workoutId } }); else if (planDay) { const id = startPlanWorkout(planDay.day); if (id) router.push({ pathname: '/active/[id]', params: { id } }); } else pushWorkout(workout.id); }} style={s.heroStart}><Text style={s.heroStartText}>{state.activeWorkout ? 'Continue active workout  →' : 'Start workout now  →'}</Text></Pressable></Card>
