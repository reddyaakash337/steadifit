...
export function Heading({ children, size = 28, style }: PropsWithChildren<{ size?: number; style?: TextStyle }>) { return <Text style={[s.heading, { fontSize: size }, style]}>{children}</Text>; }
export function Copy({ children, style }: PropsWithChildren<{ style?: TextStyle }>) { return <Text style={[s.copy, style]}>{children}</Text>; }
...
export function Card({ children, onPress, style }: PropsWithChildren<{ onPress?: () => void; style?: ViewStyle }>) {
  ...
  return onPress ? <Pressable onPress={onPress} style={({ pressed }) => pressed ? s.pressed : undefined}>{body}</Pressable> : body;
}
...
export function SectionTitle({ title, action, onPress }: { title: string; action?: string; onPress?: () => void }) {
  return <View style={s.section}><Text style={s.sectionText}>{title}</Text>{action ? <Text accessibilityRole="button" onPress={onPress} style={s.link}>{action}</Text> : null}</View>;
}
...
const s = StyleSheet.create({
  ...
  section: { marginTop: 18, marginBottom: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, sectionText: { color: C.ink, fontFamily: 'InterBold', fontSize: 14 }, link: { color: C.accent, fontFamily: 'InterSemiBold', fontSize: 12, padding: 4 },
